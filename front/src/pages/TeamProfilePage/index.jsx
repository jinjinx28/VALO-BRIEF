...
export default function TeamProfilePage() {
  ...
  useEffect(() => {
    let active = true;
    fetchTeamProfile(teamTag).then((data) => { if (active) setTeam(data); });
    return () => { active = false; };
  }, [teamTag]);

  if (!team) return <p style={{ color: 'var(--text-2)' }}>불러오는 중...</p>;

  return <TeamProfileBody team={team} />;
}

export function TeamProfileBody({ team }) {
  return (
    <>
      <ProfileHeader type="team" name={team.name} tag={team.tag} division={team.division} />
      ...
      <div className="mh-grid">
        <div>
          <div className="mh-box">
            ...
            <div className="wl-legend">
              <span><span className="dot" style={{ background: 'var(--win)' }} />{team.recentSummary.wins}승</span>
              <span><span className="dot" style={{ background: 'var(--lose)' }} />{team.recentSummary.losses}패</span>
            </div>
        ...
        <TeamMatchHistoryList matches={team.matchHistory} total={10} />
      </div>
    </>
  );
}
