...
export default function MyTeamAnalysisPage() {
  ...
  useEffect(() => {
    if (activeTab === '통계' && !stats) fetchMyTeamStats().then(setStats);
  ...
  }, [activeTab, stats, analysis, aiReport]);
  ...
  return (
    <>
      ...
      {activeTab === '통계' ? (stats ? <StatsTab stats={stats} /> : <Loading />) : null}
      {activeTab === '개인 분석' ? <PlayerAnalysisTab /> : null}
      {activeTab === '팀 분석' ? (analysis ? <TeamAnalysisTab analysis={analysis} /> : <Loading />) : null}
      {activeTab === 'AI 리포트' ? (aiReport ? <AiReportTab report={aiReport} teamName={myTeamProfileMock.name} /> : <Loading />) : null}
    </>
  );
}

function Loading() {
  return <p style={{ color: 'var(--text-2)' }}>불러오는 중...</p>;
}
