...
/**
 * data: { trade1v1, trade1v2, skills:[{name,engageRate,successRate}],
 *   duelistVsDuelist:{us,them} 또는 duelistCompare:{me,opponent}, sentinelCompare: 'advantage'|'disadvantage' }
 * ourLabel/theirLabel: 좌우 라벨 커스터마이즈 (개인 상세에서는 선수 이름 vs 상대 포지션명 등)
 */
export default function EngagementInfoBlock({ data, title = '③ 교전 정보', tradeTitle = '트레이드 성공률', ourLabel = '우리팀', theirLabel = '상대팀' }) {
  ...
  return (
    <div className="analysis-row">
      ...
      <div className="duel-compare-block" style={{ marginBottom: 0 }}>
        <div className="duel-compare-title">감시자 포지션 비교</div>
        <div className="sentinel-compare">
          {ourLabel} 감시자 지표{' '}
          <b style={{ color: data.sentinelCompare === 'advantage' ? 'var(--win)' : 'var(--lose)' }}>
